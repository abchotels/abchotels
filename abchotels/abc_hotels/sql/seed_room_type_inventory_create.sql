...
BEGIN
  ...
  WHILE v_win_s <= v_end DO
    ...
    INSERT IGNORE INTO `tabRoom Type Inventory`
      (name, room_type, rate_code, for_date, occupied_count)
    SELECT
      LEFT(CONCAT(p_name_prefix, p.room_type, '-', p.rate_code, '-', c.for_date), 140) AS name,
      p.room_type,
      p.rate_code,
      c.for_date,
      0
    FROM (
      SELECT rt.name AS room_type, rc.rate_code
      FROM `tabRoom Type` rt
      JOIN `tabRoom Type Rate Code` rc
        ON rc.parent = rt.name
       AND rc.parenttype = 'Room Type'
       AND rc.parentfield = 'rate_codes'
      WHERE rc.rate_code IS NOT NULL AND rc.rate_code <> ''
    ) AS p
    JOIN dim_date c
